...
func TestAMissingSecretIsTheNodesFaultWhenItIsAPermissionProblem(t *testing.T) {
	...
	err := errors.New("resolve secret DATABASE_URL: permission denied")
	if got := classifyStartError(err); got != FaultNode {
...
func TestAMissingSecretIsTheAppsFaultWhenItDoesNotExist(t *testing.T) {
	...
	err := errors.New("resolve secret DATABASE_URL: not found")
...
func classifyStartError(err error) Fault {
	...
	case strings.Contains(s, "resolve secret") && strings.Contains(s, "permission denied"):
		return FaultNode
	case strings.Contains(s, "resolve secret") && strings.Contains(s, "not found"):
		return FaultApp
	default:
		return FaultUnknown
	}
...
- [ ] **Step 1: Add the report type and the retained map**
...
and extend the body that `fromControlPlane` marshals. It currently marshals `s.Identity`; give `Source` a `Report func() []ProcessFault` callback and marshal an anonymous struct embedding `NodeIdentity` plus `Processes []ProcessFault \`json:"processes,omitempty"\``. `omitempty` is what makes absent-vs-empty work on the wire.
...
- [ ] **Step 2: The write, and the guard**
...
- [ ] **Step 2: Emit it, but only when corroborated**
