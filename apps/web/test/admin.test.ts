...
test("an exact email entry grants access, case- and space-insensitively", () => {
  const entries = [{ email: "ops@luwo.ai", domain: null }];
  assert.equal(isAdmin("ops@luwo.ai", entries), true);
  assert.equal(isAdmin("  OPS@Luwo.AI  ", entries), true);
  assert.equal(isAdmin("other@luwo.ai", entries), false);
});
...
test("a domain entry grants access to that whole domain and no other", () => {
  ...
  assert.equal(isAdmin("anyone@luwo.ai", entries), false);
});
...
test("malformed addresses are refused rather than matched loosely", () => {
  const entries = [{ email: null, domain: "luwo.ai" }];
  assert.equal(isAdmin("", entries), false);
  assert.equal(isAdmin("luwo.ai", entries), false);
  assert.equal(isAdmin("@luwo.ai", entries), false);
});
