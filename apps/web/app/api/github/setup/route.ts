...
export async function GET(req: Request): Promise<Response> {
  ...
  // Where they started. An allow-list of two rather than a path from the query
  // string — see `returnPathFromCallback`.
  const home = returnPathFromCallback(url);
  ...
  if (!decision.ok) return back(req, { github_error: decision.reason, ...named }, home);
  ...
  if (!workspaceId) return back(req, { github_error: "no-workspace", ...named }, home);
  ...
  try {
    ...
    if (!mine) return back(req, { github_error: "taken", ...named }, home);
    return back(req, { connected: account.login, ...named }, home);
  } catch (e) {
    ...
    return back(req, { github_error: kind, ...named }, home);
  }
}
